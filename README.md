...
```bash
insurance$ sam local invoke HelloWorldFunction --event events/event.json
```
...
insurance$ curl http://localhost:3000/
```
...
```bash
insurance$ sam logs -n HelloWorldFunction --stack-name insurance --tail
```
...
```bash
insurance$ pip install pytest pytest-mock --user
insurance$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name insurance
```
